...
- (NSString*)authToken { return [self defaultValueForKey:GITHUB_TOKEN_KEY]; }
...
- (NSString *)localUser { return [self defaultValueForKey:USER_NAME_KEY]; }
...
- (NSString *)latestReceivedEventEtag { return [self defaultValueForKey:RECEIVED_EVENT_ETAG]; }
...
{
	NSDate *d = [self defaultValueForKey:RECEIVED_LATEST_EVENT_DATE];
...
- (NSString *)latestUserEventEtag { return [self defaultValueForKey:LATEST_USER_EVENT_ETAG]; }
...
{
	NSDate *d = [self defaultValueForKey:LATEST_USER_EVENT_DATE];
	if(!d) d = [NSDate distantPast];
	return d;
}
...
{
	NSString *value = [self defaultValueForKey:API_FRONTEND_SERVER];
...
- (NSString *)apiBackEnd
{
	NSString *value = [self defaultValueForKey:API_BACKEND_SERVER];
...
- (NSString *)apiPath
{
	NSString *value = [self defaultValueForKey:API_SERVER_PATH];
	if(value==nil) value = @"";
	return value;
}
